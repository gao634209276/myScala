...
/**
  ...
  * 特别是，这意味着那个语言支持将函数当作参数传递给其他的函数，从其他的函数里返回出值，并且可以将他们设定为变量，或者将他们存储在数据结构中。
  * Scala的函数是头等函数。
  * 你不仅可以定义和调用函数，还可以把它们写成匿名的函数字面量，并把它们作为值传递。
  * 函数字面量被编译进类，并在运行期间实例化为函数值。
...
object Chapter8 {
  ...
  /**
    ...
    * 任何带有自由变量的函数字面量，如(x: Int) => x + more, 被称为开放项
    *
    */
  def fun12() {
    println("000000")
