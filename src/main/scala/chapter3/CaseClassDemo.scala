 ...
 */
/**
  * scala解决了java语言中存在的这个问题，scala解决这一问题的利器就是模式匹配，
  * 上面的java代码可以利用scala语言的模式匹配来避免，代码如下：
  */
...
/*
上述scala代码展示了如何使用scala中的模式匹配，它的实现方式是通过match关键字与 case X=>的方式实现的，
其中case _表示除了 case 10,case 50,case 80的其余匹配，类似于java中的default。
但scala语言中提供了更为灵活的匹配方式，如：
 */
...
/**
  ...
  */
//抽象类Person
abstract class Person27

//case class Student
case class Student9(name: String, age: Int, studentNo: Int) extends Person27

//case class Teacher
case class Teacher6(name: String, age: Int, teacherNo: Int) extends Person27

//case class Nobody
case class Nobody(name: String) extends Person27

object CaseClassDemo {
	def main(args: Array[String]): Unit = {
		//case class 会自动生成apply方法，从而省去new操作
		val p: Person27 = Student9("john", 18, 1024)
		//match case 匹配语法
		p match {
			case Student9(name, age, studentNo) => println(name + ":" + age + ":" + studentNo)
			case Teacher6(name, age, teacherNo) => println(name + ":" + age + ":" + teacherNo)
...
/**
  ...
  */
object CaseClassTest1 extends App {
	//toString方法演示
	val s = Teacher6("john", 38, 1024) // Teacher(john,38,1024)
	...
	s1.toString
	//带一个参数的copy方法
	s1.copy(name = "stephen") //Teacher = Teacher(stephen,38,1024)
	//带二个参数的copy方法
	s1.copy(name = "stephen", age = 58) //Teacher = Teacher(stephen,58,1024)
	//带三个参数的copy方法
	s1.copy(name = "stephen", age = 58, teacherNo = 2015)

	//Teacher = Teacher(stephen,58,2015)
...
case class SchoolClass(classDescription: String, persons: Person27*)

//下列代码给出的是其模式匹配应用示例
object CaseClassTest2 {
	def main(args: Array[String]) {

		val sc = SchoolClass("学途无忧网Scala培训班", Teacher6("摇摆少年梦", 27, 2015), Student9("摇摆少年梦", 27, 2015))
		sc match {
			case SchoolClass(_, _, Student9(name, age, studetNo)) => println(name)
...
/**
  * 3 sealed case class
  * 在进行模式匹配的时候，有些时候需要确保所有的可能情况都被列出，
  * 此时常常会将case class的超类定义为sealed（密封的) case class，
  * 如：
  */
//Person最前面加了个关键字sealed
sealed abstract class Person28

case class Student10(name: String, age: Int, studentNo: Int) extends Person28

case class Teacher7(name: String, age: Int, teacherNo: Int) extends Person28

case class Nobody2(name: String) extends Person28

case class SchoolClass2(classDescription: String, persons: Person28*)

object CaseClassTest3 {
	def main(args: Array[String]): Unit = {
		val s: Person28 = Student10("john", 18, 1024)
		//这边仅仅给出了匹配Student的情况，在编译时
		//编译器会提示
		//match may not be exhaustive. It would fail on the following inputs: Nobody(_), Teacher(_, _, _)
		/*s match {
			case Student10(name, age, studentNo) => println("Student")
		}*/
		...
		s match {
			case Student10(name, age, studentNo) => println("Student")
			case Teacher7(name, age, studentNo) => println("Teacher")
			case Nobody2(name) => println("Nobody")
		}
	}
}
...
/**
  ...
  *
  * 在实际中，case class除了在模式匹配时能发挥其强大的威力之外，
  * 在进行其它应用时，也显示出了其强大的功能，
  * 下面给出case class在SparkSQL中的应用，旨在说明case class在实际应用中的重要地位。
  * http://blog.csdn.net/lovehuangjiaju/article/details/46946533
  */